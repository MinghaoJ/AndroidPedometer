...
public class LineGraphView extends View
{
    ...
    private final int AXIS_WIDTH = 250;
    ...
    private final int AXISWIDTH = 4;
    private final int LINEWIDTH = 2*AXISWIDTH;
    ...
    public LineGraphView(Context context, int dataWidth, List<String> labels) {
        ...
        graphPaint.setStrokeWidth(AXISWIDTH);
    }
    ...
    public void setColors(int[] colors)
    {
        for(int i = 0; i < Math.min(labels.size(), colors.length); i++) {
            ...
            linePaints.get(i).setStrokeWidth(LINEWIDTH);
        }
    }
    ...
    private void prepForData(Canvas canvas)
    {
        ...
        xScale = WIDTH / (maxDataWidth + 1);
        yScale = (HEIGHT / 2) / 2;
        //xScale = WIDTH / (points.size()+1);
        //yScale = (HEIGHT / 2) / maxY;

        canvas.drawText(String.format("%.2f", maxY) + " m/s^2", 0, TEXTSIZE, graphPaint);
        canvas.drawText("-" + String.format("%.2f", maxY) + " m/s^2", 0, HEIGHT, graphPaint);

        for(int i = 0; i < labels.size(); i++){
            canvas.drawText(labels.get(i) + ":", 0, 3*TEXTSIZE + i*(2*TEXTSIZE), graphPaint);
            canvas.drawLine(0, 3.5f*TEXTSIZE + 0.5f*LINEWIDTH + i*(2*TEXTSIZE),
                    AXIS_WIDTH - 20, 3.5f*TEXTSIZE + 0.5f*LINEWIDTH + i*(2*TEXTSIZE), linePaints.get(i));
        }
    }
